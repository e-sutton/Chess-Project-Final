...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
	 ...
	 * which case it can move either one or two squares or it has already moved
	 * and the it can only one square down the board. The Pawn can also take an
	 * opponent piece in a diagonal movement. and if it makes it to the bottom
	 * of the board it turns into a Queen (this should be handled where the move
	 * is actually being made and not in this method).
	 */
	private Stack<Move> getWhitePawnSquares(int x, int y, String piece) {
		Square startingSquare = new Square(x, y, piece);
		Stack<Move> moves = new Stack<Move>();
		Move validM, validM2, validM3, validM4;
		int startYPos = y;// start position for the pawn
		int tmpx1 = x + 1;
		int tmpx2 = x - 1;
		int tmpy1 = y + 1;// allow to move one square
		int tmpy2 = y - 1;
	 ...
	 * the squares surrounding the square that was submitted to the method. The
	 * method checks the grid below:
	 *
	 *
	...
	/*
	 * The getKingSquares method takes as an input any coordinates from a square
	 * and returns a stack of all the possible valid moves that the WhiteKing
	 * can move to.
	 *
	 * So lets consider how the King can move. The King can essentially move in
	 * any direction as long as there is not another king in an adajacent square
	 * to were the king lands. Additionally, the King can only move one square
	 * at a time.
	 *
	 * To support this method we will also create a helper method called
	 * checkSurroundingSquares(Square s){, see above that returns a Boolean
	 * value to let us know if a supplied square will be adajacent to another
	 * square with a BlackKing present. Essentially if we consider that the
	 * board is a set of squares with coordinates for each square (x, y), this
	 * allows us to identify the possible squares that we should be
	 * investigating, see below;
	 *
	 * _|___________|_________|___________|_ | | | | |(x-1, y-1) |(x, y-1)
	 * |(x+1, y-1) | _|___________|_________|___________|_ | | | | |(x-1, y) |
	 * (x, y) |(x+1, y) | _|___________|_________|___________|_ | | | | |(x-1,
	 * y+1) | (x, y+1)|(x+1, y+1) | _|___________|_________|___________|_ | | |
	 * |
	 *
	 *
	 * This shows us that for a single square with coordinates of (x, y) we need
	 * to check eight other potential squares. Remember we only need to check
	 * squares and contsruct moves if the movement (Piece on a Square --> a new
	 * Square) is going to be placing the piece back on the board, if we are not
	 * taking our own piece and if the resulting landing square is not adjacent
	 * to the enemy King.
	 */
	private Stack<Move> getKingSquares(int x, int y, String piece) {
		...
		/*
		 * If we consider the grid above, we can create three different columes
		 * to check. - if x increases by one square, using the variable tmpx1
		 * (x+1) - if x decreases by one square, using the variable tmpx2 (x-1)
		 * - or if x stays the same.
		 */
		if (!((tmpx1 > 7))) {
			/*
			 * This is the first condition where we will be working with the
			 * column where x increases. If we consider x increasing, we need to
			 * make sure that we don't fall off the board, so we use a condition
			 * here to check that the new value of x (tmpx1) is not greater than
			 * 7.
			 *
			 * From the grid above we can see in this column that there are
			 * three possible squares for us to check in this column: - were y
			 * decreases, y-1 - were y increases, y+1 - or were y stays the same
			 *
			 * The first step is to construct three new Squares for each of
			 * these possibilities. As the unchanged y value is already a
			 * location on the board we don't need to check the location and can
			 * simply make a call to checkSurroundingSquares for this new
			 * Square.
			 *
			 * If checkSurroundingSquares returns a positive value we jump
			 * inside the condition below: - firstly we create a new Move, which
			 * takes the starting square and the landing square that we have
			 * just checked with checkSurroundingSquares. - Next we need to
			 * figure out if there is a piece present on the square and if so
			 * make sure that the piece is an opponents piece. - Once we make
			 * sure that we are either moving to an empty square or we are
			 * taking our opponents piece we can push this possible move onto
			 * our stack of possible moves called "moves".
			 *
			 * This process is followed again for the other temporary squares
			 * created.
			 *
			 * After we check for all possoble squares on this column, we repeat
			 * the process for the other columns as identified above in the
			 * grid.
			 */
	...
	private Stack<Move> getQueenMoves(int x, int y, String piece) {
		...
		Move tmp;
		/*
		 * The Queen is a pretty easy piece to figure out if you have completed
		 * the Bishop and the Rook movements. Either the Queen is going to move
		 * like a Bishop or its going to move like a Rook, so all we have to do
		 * is make a call to both of these methods.
		 */
	...
	/*
	 * Method to return all the squares that a Rook can move to. The Rook can
	 * either move in an x direction or in a y direction as long as there is
	 * nothing in the way and it can take its opponents piece but not its own
	 * piece. As seen in the below grid the Rook can either move in a horizontal
	 * direction (x changing value) or in a vertical movement (y changing
	 * direction)
	 *
	...
	private Stack<Move> getRookMoves(int x, int y, String piece) {
		...
		Move validM, validM2, validM3, validM4;
		/*
		 * There are four possible directions that the Rook can move to: - the x
		 * value is increasing - the x value is decreasing - the y value is
		 * increasing - the y value is decreasing
		 *
		 * Each of these movements should be catered for. The loop guard is set
		 * to incriment up to the maximun number of squares. On each iteration
		 * of the first loop we are adding the value of i to the current x
		 * coordinate. We make sure that the new potential square is going to be
		 * on the board and if it is we create a new square and a new potential
		 * move (originating square, new square).If there are no pieces present
		 * on the potential square we simply add it to the Stack of potential
		 * moves. If there is a piece on the square we need to check if its an
		 * opponent piece. If it is an opponent piece its a valid move, but we
		 * must break out of the loop using the Java break keyword as we can't
		 * jump over the piece and search for squares. If its not an opponent
		 * piece we simply break out of the loop.
		 *
		 * This cycle needs to happen four times for each of the possible
	...
	/*
	 * Method to return all the squares that a Bishop can move to. As seen in
	 * the below grid, the Bishop can move in a diagonal moement. There are
	 * essentially four different directions from a single square that the
	 * Bishop can move along. The Bishop can move any distance along this
	 * diagonal as long as there is nothing in the way. The Bishop can also take
	 * an opponent piece but cannot take its own piece.
	 *
	 *
	 * _|_____________|___________|_________|___________|___________|_ | | | | |
	 * | | | | | | |
	...
	private Stack<Move> getBishopMoves(int x, int y, String piece) {
		...
		Move validM, validM2, validM3, validM4;
		/*
		 * The Bishop can move along any diagonal until it hits an enemy piece
		 * or its own piece it cannot jump over its own piece. We need to use
		 * four different loops to go through the possible movements to identify
		 * possible squares to move to. The temporary squares, i.e. the values
		 * of x and y must change by the same amount on each iteration of each
		 * of the loops.
		 *
		 * If the new values of x and y are on the board, we create a new square
		 * and a new move (from the original square to the new square). We then
		 * check if there is a piece present on the new square: - if not we add
		 * the move as a possible new move - if there is a piece we make sure
		 * that we can capture our opponents piece and we cannot take our own
		 * piece and then we break out of the loop
		 *
		 * This process is repeated for each of the other three possible
		 * diagonals that the Bishop can travel along.
		 *
		 */
		for (int i = 1; i < 8; i++) {
			int tmpx = x + i;
	 ...
	 * possibly the simplest piece to get possible movements from. The Knight
	 * can essentially move in an L direction from any square on the board as
	 * long as the landing square is on the board and we can take an opponents
	 * piece but not our own piece.
	 */
	...
	/*
	 * The method printStack takes in a Stack of Moves and prints out all
	 * possible moves.
	 */
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (possible) {
			...
			if (pieceName.contains("Bishop")) {
				/*
				 * The Bishop is a simple piece and has a very long range so
				 * usually is used to attack from far away. It moves along the
				 * diagonal as long as nothing is in the way to its destination.
				 * Like all other Chess pieces it can take its opponent piece
				 * but cannot take its own piece.
				 */
				...
				if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
					...
					validMove = true;
					/*
					 * The condition to check for a validMove simple asks is the
					 * x movement is the same as the y movement, if so we are
					 * moving along a diagonal. We then need to identify which
					 * diagonal and make sure there is nothing in the way.
					 */
					if (xMovement == yMovement) {
						...
						} // end of the conditions to check the appropriate
							// diagonal to see if there is a piece in the way.
						if (inTheWay) {
							validMove = false;
						} else {
							/*
							 * This is cool there is nothing in the way and we
							 * just need to make sure that we are not able to
							 * take our own piece.
							 */
							...
							} else { // There is no piecePresent on the square
				...
				/*
				 * The Rook moves along the x direction or the y direction any
				 * number of squares as long as there is nothing in its way. It
				 * can take an opponents piece and as you know it cannot take
				 * its own piece.
				 */
				Boolean intheway = false;
				if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
					...
					/*
					 * The following condition simply asks if we have either a
					 * movement in the x direction or in the y direction.
					 */
					if (((xMovement != 0) && (yMovement == 0)) || ((xMovement == 0) && (yMovement != 0))) {
						if (xMovement != 0) { // we have a movement in the x
												// direction
							if (startX - landingX > 0) {
								/*
								 * We need to check all the squares up to the
								 * landing square to make sure nothing is in the
								 * way.
								 */
								for (int i = 0; i < xMovement; i++) {
						...
						if (intheway) {
							/*
							 * if we have found that there is something in the
							 * way to the square we are trying to move to we
							 * simply set the validMove boolean to false...
							 */
							validMove = false;
						} else {
							/*
							 * okay so we have established that there is nothing
							 * in the way and we want to make try and move the
							 * piece. The other condiion that we need to check
							 * is to see if there is a piece at the landing
							 * square that we want to move to.
							 */
							if (piecePresent(e.getX(), (e.getY()))) {
								if (pieceName.contains("White")) {
									if (checkWhiteOponent(e.getX(), e.getY())) {
										validMove = true;
										/*
										 * Every time we make a take a piece we
										 * need to check if the piece that has
										 * been captured is a King.
										 */
				...
				/*
				 * We know that the Knight can move in an L direction. It means
				 * that if the xMovement == 1 then the yMovement must equal 2
				 * and also the other way around... We need to check the square
				 * that we are moving to and make sure that if there is a piece
				 * present that its not our own piece...
				 */
				...
				if ((xMovement == 0) && (yMovement == 0)) {
					...
					if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
						...
						if ((xMovement > 1) || (yMovement > 1)) {
							...
							/*
							 * The Kind can only move one square at a time but
							 * also needs to make sure that there is at least
							 * one square between both Kings once the move has
							 * concluded.
							 */
							if ((getPieceName((e.getX() + 75), e.getY()).contains("King"))
								...
								/*
								 * We need to make sure that if there is a piece
								 * on the potential landing square that the King
				...
				if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
					...
					/*
					 * The Queen can either move like the Rook or a Bishop...You
					 * already have this code working!
					 *
					 * The first check is to see if the Queen is making a valid
					 * Bishop Like move...
					 */
					if (xMovement == yMovement) {
						if ((startX - landingX < 0) && (startY - landingY < 0)) {
						...
						if (inTheWay) {
							...
							/*
							 * If there is nothing in the way when moving the
							 * Queen like a Bishop, we have two cases: - there
							 * is a piece on the square we are trying to move to
							 * and we need to make sure that its an enemy piece
							 * - or there is nothing in the way and everything
							 * is cool
							 */
							if (piecePresent(e.getX(), (e.getY()))) {
								if (pieceName.contains("White")) {
					...
					} // end of the condition where the Queen is moving like a
						// Bishop
					else if (((xMovement != 0) && (yMovement == 0)) || ((xMovement == 0) && (yMovement != 0))) {
						/*
						 * This is the condition that handles the Queen
						 * movements that are moving like a Rook
						 */
						if (xMovement != 0) {
						...
						if (inTheWay) {
							...
							/*
							 * This is cool and just like before there is now
							 * nothing in the way and the Queen should be able
							 * to make the movement. We need to check if there
							 * is something on the square that we are moving to
							 * and if so make sure its an enemy piece and if
							 * there is nothing on the square that we are moving
							 * to ... make the move.
							 */
							if (piecePresent(e.getX(), (e.getY()))) {
								if (pieceName.contains("White")) {
				...
				if (startY == 6) { // This is were the pawn is making its first
									// move....
					/*
					 * if the pawn is makeing ites first movement.... the pawn
					 * can either move one square or two squares...in the Y
					 * direction as long as we are moving up the board!!! and
				...
				} else {// This is were the pawn is making all subsequent
						// moves...
					if (((yMovement == 1)) && (startY > landingY) && (xMovement == 0)) {
						if (!piecePresent(e.getX(), e.getY())) {
							validMove = true;
							/*
							 * We need a variable to inform us that the Black
							 * Pawn has reached the top of the board and can now
							 * trun into a Black Queen
							 */
							if (landingY == 0) {
								progression = true;
							}
				...
				if (startY == 1) {
					if (((xMovement == 0)) && ((yMovement == 1) || ((yMovement) == 2))) {
						if (yMovement == 2) {
							...
							&& (startY < landingY)) {
						/*
						 * This is the condition were we are checking for a move
						 * to take a piece when the white pawn is making its
						 * first move. We check the following: - that there is a
						 * piece present, - we are moving in a diagonal, - the
						 * movement is only one square, - we are moving down the
						 * board
						 */
						if (checkWhiteOponent(e.getX(), e.getY())) {
							validMove = true;
				...
				} // End of the movements for when the pawn is making its first
					// move.
				else {
		...
		if (!validMove) {
			...
			panels.add(pieces);
		} else { // this is the condition where we have a valid move and need to
					// visually show it.
			if (white2Move) {
				white2Move = false;
			} else {
